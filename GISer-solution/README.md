...
3. geoserver扩展更适合开发和生成地图服务相关的功能。GIS的空间分析功能一开始通过geoserver扩展插件实现，后来发现扩展插件的后台程序主要作用是数据传输，最主要环节的GIS空间分析环节是在空间数据库PostGIS中实现的，而geoserver的扩展开发环境比较复杂，不如自己写的java后台好维护。
...
1. 地图坐标偏移问题，之前通过封装，在对外接口和地图之间形成了一个坐标适配层，解决了坐标偏移问题。如果放开原生接口后，还是否能对外用统一的坐标？
...
基于这一点，我们这一版的帮助文档以示例为线索，示例中的注释保证完善清晰。将示例代码中用到的方法给出类参考链接，再将leaflet的类参考文档进行翻译，放到平台中。

放开leaflet原生接口，不再对接口进行封装。

将geoserver扩展功能迁移到java后台，geoserver扩展的优势是可以直接调用geoserver的资源和功能，它更适合开发和生成地图服务相关的功能，目前还没有相关的需求，等有时再使用扩展开发模式。

### 经验总结

1. leaflet类参考文档的翻译工作做的不好，没翻译几页，但发现对大家完全没有影响，两个原因，一是大家都是程序员，简单的英文文档基本能看懂，二是大家其实基本不咋看文档，更多的是看示例，示例没有的，会直接问我们，最后文档其实只要我们自己在看（捂脸）
2. 这一版我们维护的模式是，先把常用的示例都写上，后续有用户在示例中没有找到自己需要的，过来找我们，简单的功能我们就直接用原生api写个示例，也不用封装，直接把示例挂到帮助网站上。如果是复杂的功能再考虑封装。有人问问题时，就尽量以示例的方式记录下来。
3. 要制定操作流程规范，并通过工具，让写示例并更新到网站这件事变简单，太复杂会不愿意去写示例。
